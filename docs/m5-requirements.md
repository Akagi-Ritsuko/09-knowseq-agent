...
- 空闲低电平段（无语音，实测 RMS -35.6dBFS）可致 asr_infer 崩溃 exit 3221226505（0xC0000409 STATUS_STACK_BUFFER_OVERRUN，Windows fail-fast），崩溃点在模型/音频加载成功后的 Decoding 阶段——与 seg007 英文幻觉同族，均属 VibeASR 对低电平输入的推理健壮性边界（外部二进制，不在修复范围）；失败经 meeting_source `_failed` 会话内隔离，不重试、不落空稿、不扩散（T-509 收尾期 22:39 空录段实证）。
