...
## M4 交互层 —— 已完成（2026-09-03）

> 详细需求与待办：见 [m4-requirements.md](m4-requirements.md)（REQ-401~411）与 [tasks.md](tasks.md)（T-401~411 / T-011 / T-115）。
> 2026-09-03 T-411 端到端验收通过：真实环境闭环「提问→带引用回答→跳转来源→查看图谱」走查通过（React 19 + Vite + TS 七页：问答/图谱/知识库/编译/素材/采集控制/设置；sigma.js v3 图谱 337 节点/544 关系真实渲染）；能力覆盖逐项核对无回退（采集开关/素材标记/知识编辑删除带确认/设置持久化/编译链路/大脑面板）；旧 `app/web/static/` 已删除，dist 托管 + SPA fallback，旧路径呈前端 404 页不可达；FastAPI 后端无破坏性变更（仅补管理 API）。
...
- **验收**：会议音频经悬浮按钮开始/结束 → `inbox/meeting/` 产出 system-audio 转写稿并可编译问答；壳内控制台闭环与浏览器等价；右键/托盘/悬浮三入口均可控制采集；后端随系统自启；演示脚本从零跑通。
